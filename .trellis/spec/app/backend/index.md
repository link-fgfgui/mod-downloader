...
- Inject release identity through `APP_VERSION` and Go `-X`; see
  [Build Version](./build-version.md). Production builds use
  `.github/workflows/build.yml` to inject the version and the compile-time
  default CurseForge key together (see
  [Default CurseForge API Key](../../core/backend/default-curseforge-api-key.md)).
